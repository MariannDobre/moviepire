...
export default function DiaryMovie({ movie }) {
  ...
  return (
    <div className='w-full h-[420px] flex gap-6 relative'>
      <button
        type='button'
        onClick={removeFromDiary}
        disabled={isRemovingFromDiary}
        className='group absolute top-0 right-0 w-10 h-10 outline-none border border-neutral-500 cursor-pointer disabled:cursor-not-allowed disabled:opacity-75 bg-transparent flex items-center justify-center rounded-md shadow-sm hover:border-red-700 focus-visible:border-red-700 hover:bg-red-100/10 focus-visible:bg-red-100/10 transition-all duration-500'
      >
        {isRemovingFromDiary ? (
          <SmallLoader
            color='text-red-500'
          ...
          <span className='w-full h-full flex items-center justify-center text-base text-red-500 group-hover:text-red-700 group-focus-visible:text-red-700 transition-all duration-500'>
            <FaTrashAlt />
          </span>
        )}
      </button>
      ...
      <LazyLoadImage
        className='w-[280px] h-full object-cover rounded-t-lg drop-shadow-sm opacity-85'
        width={280}
        src={moviePoster}
      ...
      <div className='w-[calc(100%-280px-24px)] h-auto flex flex-col gap-3'>
        {/* HEADING */}
        <MovieHeading
          id={id}
          movieName={movieName}
